...
// <Route> :
// ex. <Route path='/page1' element={<ReactRouterPage1 />} />
// - URL 패턴에 따라서 랜더링하고자 하는 컴포넌트를 지정하는 컴포넌트
// - path 속성 : URL 패턴 지정 ex. path='/page1'
// - element 속성 : 랜더링할 컴포넌트 지정  ex. element={<ReactRouterPage1 />}
// - index 속성 : 현재 패턴의 기본 라우터로 지정

// http://loaclhost:3000
// http://loaclhost:3000/router
export default function ReactRouterLibrary() {

  // - <a> 요소로 페이지 전환을 하게되면 새로운 요청을 보내게 되어 반드시 페이지가 새로고침 됨
  // - 페이지가 새로고쳐지면 상태 유지가 안됨

  // <Link> :
  // - 사용자가 해당 웹 애플리케이션 내에서 다른 페이지로 이동할 수 있도록 하는 컴포넌트
  // - a 요소의 href로 페이지 전환을 하게되면 새로운 페에지로 전향되는 반면, <Link>는 새로고침을 하지 않고 URL만 변경
  // - 다른 웹 애플리케이션으로 이동하는 작업은 <Link>로 수행되지 않음
  ...
  // useNavigate() :
  // - Navigate 함수를 반환하는 훅 함수
  // - Navigate 함수는 특정 조건에 따라서 URL을 변경하고자 할 때 사용할 수 있음
...
// http://loaclhost:3000/page1
// http://loaclhost:3000/router/page1
...
// http://loaclhost:3000/page2
// http://loaclhost:3000/router/page2
export function ReactRouterPage2() {
  ...
  // ex) http://localhost:3000/router/page2?time=1031
