...
// - create 함수의 매개변수로 set 인자로 받는 콜백 함수를 전달해야 함
// - 매개변수로 전달한 콜백 함수는 store 객체를 반환해야 함
// - store 객체는 상태, store 객체를 변경하는 함수가 포함됨

// - typescript에서 zustand의 crate 함수를 사용할 때는 crate 함수의 제너릭으로 store의 타입을 지정
interface Store {
  zNormal : number;
...
const useStore = create<Store>((set) => 
  ({
    ...
    // set 함수의 매개변수로 전달된 콜백 함수는 상태 객체 (store)를 반환해야함
