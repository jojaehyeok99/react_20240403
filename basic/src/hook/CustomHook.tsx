...
// Custom Hook :
// - react의 훅 함수는 반드시 함수형 컴포넌트의 코드 블럭에서만 호출 가능
...
function usePagination (totalList : string[]) {
  ...
  const COUNT_PER_PAGE = 5; // 한 페이지에 5개 게시물
  const COUNT_PER_SECTION = 10; // 한 섹션에 10개 페이지
  ...
  useEffect(() => {
    const boardList = totalList; // 이렇게 안해줄 경우 빈배열인 0의 값이 boardList로 들어감
  ...
  // 페이지를 바꿨을 때 게시물이 바뀌는 작업
  useEffect (() => {
    if(!boardList.length) return;
    changeViewList(boardList);
  }, [page]);

  // 세션이 바뀔 때 페이지 번호 변경 작업
  useEffect(() => {
    changeViewPageList();
  },[section]);
  ...
  return (
    <div>
      ...
      <div>
        ...
        {viewPageList.map((item,index) => (<span key = {index} style={{display: 'inline-block', margin : '4px', fontWeight: item === page ? 900 : 400}}
